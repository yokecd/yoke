...
func (releaser Releaser) ReleaseWasmBinary(name string) (err error) {
	...
	ociVersion := strings.TrimPrefix(path.Base(tag), "v")
...
func buildCompressedWasm(path string) (string, error) {
	...
	if err := x.Xf("go run ./cmd/yoke schematics -wasm %s set -cmd schema", []any{path}, x.Input(strings.NewReader("[-schema]"))); err != nil {
