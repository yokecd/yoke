...
func GetTakeoffParams(settings GlobalSettings, source io.Reader, args []string) (*TakeoffParams, error) {
	...
	flagset.BoolVar(&params.SendToStdout, "stdout", false, "deprecated: see yoke flightplan command instead - execute the underlying wasm and outputs it to stdout but does not apply any resources to the cluster")
	...
	flagset.StringVar(&params.Out, "out", "", "deprecated: see yoke flightplan command instead - if present outputs flight resources to directory specified, if out is '-' outputs to standard out")
