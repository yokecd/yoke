			...
			yamlView := YamlView{
				...
				Back: Nav{
					Model: func(dim tea.WindowSizeMsg) tea.Model {
						return FlightListView{Dim: dim, GK: view.GK}
					},
					Cmd: func() tea.Msg {
						return ExecMsg(func(cmds Commands) tea.Cmd { return cmds.GetFlightList(view.GK) })
					},
				},
				HeaderActions: []HeaderActionItem{
					{
						Key:         "esc",
						Description: "view " + view.GK.String(),
					},
				},
			}

			getResource := func() tea.Msg {
				return ExecMsg(func(cmd Commands) tea.Cmd {
					return cmd.GetResourceYaml(ref)
				})
			}

			return yamlView, getResource
		}
